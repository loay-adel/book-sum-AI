...
export const saveAutoBlog = async (req, res) => {
  console.log('🎯 saveAutoBlog called');
  ...
  try {
    ...
    // IMMEDIATELY respond to client - don't wait for blog save
    res.status(202).json({
      message: 'Blog save request accepted (processing in background)',
      accepted: true,
      timestamp: new Date().toISOString()
    });

    // Now process the blog save ASYNCHRONOUSLY
    console.log('🔄 Starting async blog save process...');
    
    // Use setTimeout to make it truly async (non-blocking)
    setTimeout(async () => {
      try {
        // Check for duplicates
        console.log('🔍 Checking for duplicate blog...');
        const existingBlog = await Blog.findOne({
          $or: [
            { title: { $regex: new RegExp(title, 'i') } },
            { 'bookDetails.title': { $regex: new RegExp(bookDetails?.title || title, 'i') } }
          ],
          'user.userId': userId
        });

        if (existingBlog) {
          console.log('✅ Blog already exists, skipping duplicate');
          console.log('Existing blog ID:', existingBlog._id);
          return;
        }

        console.log('✅ No duplicate found, creating new blog...');
        ...
        console.error('❌ Error in async blog save:', error);
        // Don't throw - this is background processing
      }
    }, 0); // Process in next event loop cycle

  } catch (error) {
    console.error('❌ Error in saveAutoBlog:', error);
    // Still respond with 202 since the request was valid
    res.status(202).json({
